...
chrome.action.onClicked.addListener(async tab => {
	...
	var checkerPlusElseCompose = browserButtonAction == BrowserButtonAction.CHECKER_PLUS && checkerPlusBrowserButtonActionIfNoEmail == BrowserButtonAction.COMPOSE && unreadCount == 0;
	var gmailInboxElseCompose = browserButtonAction == BrowserButtonAction.GMAIL_INBOX && gmailPopupBrowserButtonActionIfNoEmail == BrowserButtonAction.COMPOSE && unreadCount == 0;
	...
	if (browserButtonAction == BrowserButtonAction.CHECKER_PLUS_POPOUT || browserButtonAction == BrowserButtonAction.GMAIL_INBOX_POPOUT) {
		openInPopup();
	} else if (browserButtonAction == BrowserButtonAction.COMPOSE || checkerPlusElseCompose || gmailInboxElseCompose) {
		// open compose mail
		if (accounts.length) {
			accounts[0].openCompose();
		} else {
...
async function createNotification(params) { // expected args: options, buttonsWithValues, emails
    ...
    if (showNotificationDuration == "infinite") {
        if (DetectClient.isLinux()) { // refer to https://jasonsavard.com/forum/discussion/comment/34928#Comment_34928
            params.options.requireInteraction = true;
        }
    } else {
        setTimeout(async () => {
