	...
	if (!byId(tabName + "Page")) {
		...
		if (tabName == "welcome") {
			...
			onClick("#addAccount", async function () {
                ...
                if (!getAccountsWithErrors(accounts).length && accounts.length == 1 && !await canAddMultipleAccounts()) {
...
async function loadAccountsOptions(loadAccountsParams = {}) {
	...
	document.body.classList.toggle("browserButtonAction_gmailInbox", await storage.get("browserButtonAction") == BrowserButtonAction.GMAIL_INBOX);
...
async function initOnlyWithCheckerPlusPopupWarning(params) {
	...
	if (browserButtonAction == BrowserButtonAction.CHECKER_PLUS || browserButtonAction == BrowserButtonAction.CHECKER_PLUS_POPOUT) {
