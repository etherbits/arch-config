...
if (chrome.runtime.onMessage) {
	chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
		if (message.command == "profileLoaded") {
            ...
            if (userHasInteractedWithPopupDate.diffInSeconds() < -SECONDS_SINCE_LAST_INTERACTION) {
                maybeRenderAccounts();
            } else {
                const repeatLogic = () => {
...
function setAccountHeaderColor($account, color) {
    ...
    accountHeaderStyle += `background-color: ${color} !important`;
...
async function maybeRenderAccounts() {
    ...
    let previousMailIds = getMailIds();
...
async function init() {
	...
	if (browserButtonAction == BrowserButtonAction.GMAIL_INBOX || browserButtonAction == BrowserButtonAction.GMAIL_INBOX_POPOUT) {
	...
	if (previewMailId && (await storage.get("browserButtonAction") != BrowserButtonAction.GMAIL_INBOX && await storage.get("browserButtonAction") != BrowserButtonAction.GMAIL_INBOX_POPOUT)) {
        document.body.classList.add("page-loading-animation");
